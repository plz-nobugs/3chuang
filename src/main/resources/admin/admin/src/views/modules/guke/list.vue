      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt">
          ...
          <el-form-item label="顾客姓名">
            <el-input v-model="searchForm.gukexingming" placeholder="顾客姓名" clearable></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="success" icon="el-icon-search" @click="search()">查询</el-button>
            <el-button type="success" icon="el-icon-plus" @click="addOrUpdateHandler()">新增</el-button>
          </el-form-item>
        </el-row>
      </el-form>
        ...
        <div class="card-container">
          <div v-for="(item, index) in dataList" :key="index" class="card">
            <div class="card-header">
              <h3>{{ item.gukexingming }}</h3>
              <p>账号: {{ item.zhanghao }}</p>
            </div>
            <div class="card-body">
              <p><strong>性别:</strong> {{ item.xingbie }}</p>
              <p><strong>手机:</strong> {{ item.shouji }}</p>
              <p><strong>邮箱:</strong> {{ item.youxiang }}</p>
              <p><strong>身份证:</strong> {{ item.shenfenzheng }}</p>
              <p><strong>地址:</strong> {{ item.dizhi }}</p>
            ...
            <div class="card-footer">
              <el-button v-if="isAuth('guke','查看')" @click="addOrUpdateHandler(item.id,'info')" size="mini">详情</el-button>
              <el-button v-if="isAuth('guke','修改')" @click="addOrUpdateHandler(item.id)" size="mini">修改</el-button>
              <el-button v-if="isAuth('guke','删除')" @click="deleteHandler(item.id)" size="mini">删除</el-button>
            </div>
          </div>
        </div>
        <el-pagination
