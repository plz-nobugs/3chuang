    ...
    <div v-if="showFlag">
      ...
      <div class="table-content">
        <div class="card-container">
          <div v-for="(item, index) in dataList" :key="index" class="card" @click="cardClick(item)">
            <div class="card-header">
              <h3>{{ item.yuangongxingming }}</h3>
              <p>店主账号: {{ item.yuangongzhanghao }}</p>
            </div>
            <div class="card-body">
              <p><strong>性别:</strong> {{ item.xingbie }}</p>
              <p><strong>联系电话:</strong> {{ item.lianxidianhua }}</p>
              <!-- 修改邮箱显示，尝试显示 item.youxiang 或 item.email -->
              <p><strong>邮箱:</strong> {{ item.youxiang || item.email }}</p>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (this.searchForm.yuangongzhanghao != '' && this.searchForm.yuangongzhanghao != undefined) {
        params['yuangongzhanghao'] = '%' + this.searchForm.yuangongzhanghao + '%';
      }
